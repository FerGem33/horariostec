...
import { ArrowLeft, ArrowRight, BriefcaseBusiness, Bot, Boxes, ChevronDown, ChevronUp, Code2, Cog, Cpu, Download, Factory, GripVertical, LockKeyhole, Zap, type LucideIcon } from "lucide-react";
...
function CareerSelection({ careers, onSelect }: { careers: Career[]; onSelect: (career: Career) => void }) {
  return <div className="container page schedule-page">
    ...
    <div className="career-grid">{careers.map((career) => <button type="button" disabled={!career.teacher_count} className={`career-card career-${career.slug} schedule-career-card${career.teacher_count ? "" : " career-card-locked"}`} key={career.slug} onClick={() => career.teacher_count && onSelect(career)}><CareerIcon career={career} /><span className="career-card-content"><strong>{career.name}</strong><small>{career.teacher_count ? `${career.teacher_count} docentes · ${career.subject_count} materias` : "Aún sin datos importados"}</small></span>{career.teacher_count ? <b className="schedule-card-arrow">→</b> : <LockKeyhole className="career-lock" size={20} aria-hidden="true" />}</button>)}</div>
  </div>;
}

function StepHeader({ step, career, onBack, onStep }: { step: Step; career: Career; onBack: () => void; onStep: (step: Step) => void }) {
  const labels = ["Carrera", "Materias", "Disponibilidad", "Prioridades", "Resultados"];
  return <><div className="schedule-topline"><button type="button" className="back-link schedule-back" onClick={onBack}>← Cambiar carrera</button><span>{career.name}</span></div><div className="schedule-steps" aria-label="Progreso"><div className="schedule-step-count">0{step} / 05</div>{labels.map((label, index) => { const target = (index + 1) as Step; return <button type="button" className={target === step ? "schedule-step active" : target < step ? "schedule-step done" : "schedule-step"} key={label} disabled={target > step} onClick={() => onStep(target)} aria-current={target === step ? "step" : undefined}><i>{index + 1}</i><span>{label}</span></button>; })}</div></>;
}
...
function ResultFilter({ counts, value, onChange, allLabel, disabled = false }: { counts: number[]; value: "all" | number; onChange: (value: "all" | number) => void; allLabel?: string; disabled?: boolean }) {
  const [open, setOpen] = useState(false);
  const allOptionLabel = allLabel ?? (counts.length === 1 ? String(counts[0]) : String(counts[0] ?? 0));
  const label = value === "all" ? allOptionLabel : String(value);
  return <div className={disabled ? "result-filter disabled-filter" : "result-filter"}><span>Materias</span><button type="button" className="result-filter-button" aria-haspopup="menu" aria-expanded={open} disabled={disabled} onClick={() => setOpen((current) => !current)}>{label}<ChevronDown size={14} /></button>{open && !disabled && <div className="result-filter-menu" role="menu"><button type="button" className={value === "all" ? "selected" : ""} onClick={() => { onChange("all"); setOpen(false); }}>{allOptionLabel}</button>{counts.map((count) => <button type="button" className={value === count ? "selected" : ""} key={count} onClick={() => { onChange(count); setOpen(false); }}>{count}</button>)}</div>}</div>;
}

function ScheduleReport({ candidate, start, end, pdf = false }: { candidate: CatalogOffering[]; start: string; end: string; pdf?: boolean }) {
  const subjects = [...new Map(candidate.map((item) => [item.subject_id, item])).values()];
  const details = <div className={pdf ? "schedule-report-subjects" : "schedule-preview-subjects"}>{subjects.map((item) => <div className={`${pdf ? "schedule-report-subject" : "schedule-preview-subject"} schedule-color-border-${item.subject_id % 8}`} key={item.section_id}><strong>{item.course_code && <code>{item.course_code}</code>}{item.subject}</strong><b>{item.credits ? `${item.credits} créditos` : "Materia"} · {item.group_name ? `Grupo ${item.group_name}` : "Sin grupo"}</b><small>{item.teacher}</small></div>)}</div>;
  if (!pdf) return <div className="schedule-preview"><ScheduleGrid candidate={candidate} start={start} end={end} />{details}</div>;
  return <div className="schedule-report"><div className="schedule-report-header"><div className="schedule-report-brand"><img src="/icon.png" alt="" /><div><strong>Horarios<span>Tec</span></strong></div></div></div><ScheduleGrid candidate={candidate} start={start} end={end} />{details}<div className="schedule-report-footer"><span>Generado por HorariosTec</span><span>{new Date().toLocaleDateString("es-MX")} · {new Date().toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })}</span></div></div>;
}
...
function Results({ candidates, target, strict, onStrictChange, start, end, onBack }: { candidates: Candidate[]; target: number; strict: boolean; onStrictChange: (value: boolean) => void; start: string; end: string; onBack: () => void }) {
  ...
  const move = (delta: number) => setIndex((current) => visibleCandidates.length ? (current + delta + visibleCandidates.length) % visibleCandidates.length : 0);
  return <div className="container page schedule-page"><div className="schedule-heading"><div><div className="eyebrow">PASO 05 / RESULTADOS</div><h1>Tus opciones</h1><p>{visibleCandidates.length ? `${visibleCandidates.length} ${visibleCandidates.length === 1 ? "opción compatible" : "opciones compatibles"} encontradas${subjectFilter === "all" && strict ? ` con ${target} materias` : ""}.` : "No encontramos una combinación compatible con este filtro."}</p></div><div className="result-tools"><ResultFilter counts={counts} value={subjectFilter} allLabel={String(target)} disabled={strict} onChange={(value) => { setSubjectFilter(value); setIndex(0); }} /><label className="result-strict-toggle"><input type="checkbox" checked={strict} onChange={(event) => onStrictChange(event.target.checked)} /><span><strong>Estricto</strong><small>Exactamente la cantidad elegida</small></span></label><button type="button" className="button button-light" onClick={() => void exportPdf(pdfRef.current)} disabled={!candidate}><Download size={16} /> PDF</button></div></div>{candidate ? <><div className="result-viewer"><div className="result-navigation"><button type="button" className="result-arrow" onClick={() => move(-1)} aria-label="Horario anterior"><ArrowLeft size={19} /></button><span><strong>{String(index + 1).padStart(2, "0")}</strong> / {String(visibleCandidates.length).padStart(2, "0")}</span><button type="button" className="result-arrow" onClick={() => move(1)} aria-label="Siguiente horario"><ArrowRight size={19} /></button></div><article className="result-card open" ref={scheduleRef}><ScheduleReport candidate={candidate.offerings} start={start} end={end} /></article><div className="pdf-render-host" ref={pdfRef} aria-hidden="true"><ScheduleReport candidate={candidate.offerings} start={start} end={end} pdf /></div></div><div className="schedule-actions result-actions"><button className="button button-light" type="button" onClick={onBack}><ArrowLeft size={17} /> Volver</button><span>Usa las flechas para comparar horarios.</span></div></> : <><div className="empty-panel results-empty"><strong>Prueba con alguno de estos cambios:</strong><span>Amplía tu rango, quita una hora libre, incluye el sábado o desactiva “Cantidad estricta”.</span></div><div className="schedule-actions result-actions"><button className="button button-light" type="button" onClick={onBack}><ArrowLeft size={17} /> Volver</button></div></>}</div>;
}
...
export default function ScheduleBuilder() {
  ...
  const chooseCareer = async (next: Career) => { setCareer(next); setLoading(true); setError(""); try { const [subjectData, catalogData] = await Promise.all([api.subjects(next.slug), api.catalog(next.slug)]); setSubjects(subjectData.subjects); setOfferings(catalogData.offerings); setSelected([]); setTarget(7); setExcludedOfferings({}); setStep(2); } catch (err) { setError(err instanceof Error ? err.message : "No pudimos cargar la carrera."); } finally { setLoading(false); } };
